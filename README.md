...
> **Note:** When configuring `EMBEDDINGS_TYPE`, valid options are `HF`, `GOOGLE_GENAI`, or `GOOGLE_VERTEXAI`. If using `HF`, no API key is needed but startup is slower. If using `GOOGLE_GENAI`, ensure `GOOGLE_API_KEY` is set. If using `GOOGLE_VERTEXAI`, set `GOOGLE_APPLICATION_CREDENTIALS` to a service account key with the Vertex AI User role; see [backend/README.md](./backend/README.md).
