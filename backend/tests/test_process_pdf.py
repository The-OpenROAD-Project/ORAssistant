...
class TestProcessPDF:
    ...
    @patch('builtins.open', new_callable=mock_open, read_data='{"test.pdf": "https://example.com"}')
    ...
    @patch('src.tools.process_pdf.PyPDFLoader')
    @patch('builtins.open', new_callable=mock_open, read_data='{"doc1.pdf": "https://example1.com", "doc2.pdf": "https://example2.com"}')
    ...
    @patch('src.tools.process_pdf.PyPDFLoader')
    @patch('builtins.open', new_callable=mock_open, read_data='{}')
    def test_process_pdf_docs_logs_error_for_missing_source(self, mock_file, mock_loader, mock_logging):
    ...
    @patch('src.tools.process_pdf.PyPDFLoader')
    @patch('builtins.open', new_callable=mock_open, read_data='{"nested/path/document.pdf": "https://example.com"}')
    ...
    def test_process_pdf_docs_metadata_transformation(self):
        ...
        with patch('src.tools.process_pdf.PyPDFLoader') as mock_loader:
            with patch('builtins.open', mock_open(read_data='{"test.pdf": "https://example.com"}')):
    ...
    @patch('src.tools.process_pdf.PyPDFLoader')
    @patch('builtins.open', new_callable=mock_open, read_data='{"test.pdf": "https://example.com"}')
    def test_process_pdf_docs_uses_text_splitter(self, mock_file, mock_loader, mock_text_splitter):
        ...
        mock_loader_instance.load_and_split.assert_called_once_with(text_splitter=mock_text_splitter)

    @pytest.mark.integration
    def test_process_pdf_docs_with_realistic_data(self):
        """Test with realistic PDF metadata and content."""
        with patch('src.tools.process_pdf.PyPDFLoader') as mock_loader:
            with patch('builtins.open', mock_open(read_data='{"openroad_manual.pdf": "https://openroad.readthedocs.io/manual.pdf"}')):
                ...
                assert all(doc.metadata["source"] == "data/pdf/openroad_manual.pdf" for doc in result)
