...
class TestStreamingEndpoint:
    ...
    async def test_get_response_stream_never_yields_none(
        ...
        """Regression: non-AIMessageChunk events must not leak "None" into the client stream (issue #243, bug 2)."""
        from src.api.routers.conversations import get_response_stream
...
class TestInMemoryChatHistoryBounding:
    """Regression tests: in-memory chat_history must be bounded when USE_DB is false (issue #243, bug 4)."""
    ...
    def test_add_conversation_evicts_oldest_when_over_capacity(self):
    ...
    def test_add_conversation_stays_bounded_over_many_inserts(self):
