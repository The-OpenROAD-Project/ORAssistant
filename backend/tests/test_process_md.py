...
class TestProcessMd:
    ...
    def test_md_to_text_with_links(self):
        ...
        md_content = "Check out [OpenROAD](https://openroad.readthedocs.io/) for more info."
...
class TestLoadDocs:
    ...
    def test_load_docs_multiple_files(self, mock_md_to_text, mock_open, mock_glob):
        ...
        mock_open.return_value.__enter__.return_value.read.side_effect = ["# Content 1", "# Content 2"]
...
class TestProcessMd:
    ...
    @patch('builtins.open', create=True)
    def test_process_md_without_splitting(self, mock_open, mock_load_docs, mock_listdir, mock_exists):
        """Test processing markdown files without text splitting."""
        mock_exists.return_value = True
        mock_listdir.return_value = ['file.md']
        mock_open.return_value.__enter__.return_value.read.return_value = '{"test.md": "https://example.com"}'
        
        from langchain.docstore.document import Document
        mock_docs = [Document(page_content="Test content", metadata={"source": "test.md"})]
    ...
    @patch('builtins.open', create=True)
    def test_process_md_with_splitting(self, mock_open, mock_chunk, mock_split, mock_load_docs, mock_listdir, mock_exists):
    ...
    @patch('src.tools.process_md.os.path.exists')
    @patch('src.tools.process_md.os.listdir')
    def test_process_md_split_without_chunk_size_raises_error(self, mock_listdir, mock_exists):
    ...
    @patch('src.tools.process_md.load_docs')
    @patch('builtins.open', create=True)
    def test_process_md_missing_source_in_dict(self, mock_open, mock_load_docs, mock_listdir, mock_exists):
        """Test processing when source not found in source_list.json."""
        mock_exists.return_value = True
        mock_listdir.return_value = ['file.md']
        mock_open.return_value.__enter__.return_value.read.return_value = '{}' # Empty JSON
        
        from langchain.docstore.document import Document
        mock_docs = [Document(page_content="Test content", metadata={"source": "missing.md"})]
        mock_load_docs.return_value = mock_docs
    ...
    @patch('src.tools.process_md.os.path.exists')
    @patch('src.tools.process_md.os.listdir')
    def test_process_md_logs_error_for_empty_folder(self, mock_listdir, mock_exists, mock_logging):
    ...
    @patch('src.tools.process_md.load_docs')
    @patch('builtins.open', create=True)
    def test_process_md_logs_warning_for_missing_source(self, mock_open, mock_load_docs, mock_listdir, mock_exists, mock_logging):
    ...
    @patch('src.tools.process_md.load_docs')
    @patch('builtins.open')
    def test_process_md_realistic_workflow(self, mock_open, mock_load_docs, mock_listdir, mock_exists):
        ...
        from langchain.docstore.document import Document
        mock_docs = [
            Document(page_content="Installation content", metadata={"source": "installation.md"}),
