...
class TestHybridRetrieverChain:
    ...
    def test_create_hybrid_retriever_with_provided_vector_db(self, mock_ensemble, mock_bm25_chain, mock_mmr_chain, mock_sim_chain):
        ...
        mock_sim_instance.create_similarity_retriever.assert_called_once_with(search_k=5)
        ...
        mock_ensemble.assert_called_once_with(
            retrievers=[mock_sim_instance.retriever, mock_mmr_instance.retriever, mock_bm25_instance.retriever],
    ...
    def test_create_hybrid_retriever_with_contextual_rerank(self, mock_cross_encoder, mock_reranker, 
                                                           mock_compression, mock_ensemble, mock_bm25_chain, 
                                                           mock_mmr_chain, mock_sim_chain):
        ...
        mock_cross_encoder.assert_called_once_with(model_name="cross-encoder/ms-marco-MiniLM-L-6-v2")
        mock_reranker.assert_called_once_with(
            model=mock_cross_encoder_instance,
            top_n=5
        )
    ...
    @patch('src.chains.hybrid_retriever_chain.BM25RetrieverChain')
    @patch('src.chains.hybrid_retriever_chain.EnsembleRetriever')
    def test_create_hybrid_retriever_loads_existing_db(self, mock_ensemble, mock_bm25_chain, 
                                                      mock_mmr_chain, mock_sim_chain, mock_listdir, mock_isdir):
        ...
        mock_sim_instance.vector_db.load_db.assert_called_once_with("similarity_INST_test_db")
    ...
    @patch('src.chains.hybrid_retriever_chain.BM25RetrieverChain')
    @patch('src.chains.hybrid_retriever_chain.EnsembleRetriever')
    def test_create_hybrid_retriever_embeds_docs_when_no_db(self, mock_ensemble, mock_bm25_chain, 
                                                           mock_mmr_chain, mock_sim_chain, mock_isdir):
    ...
    def test_create_llm_chain(self, mock_passthrough, mock_parallel):
        ...
        with patch.object(chain, 'create_llm_chain', wraps=chain.create_llm_chain) as mock_super_create:
