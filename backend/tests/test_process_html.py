...
class TestProcessHTML:
    ...
    @patch('builtins.open', new_callable=mock_open, read_data='{"test.html": "https://example.com"}')
    @patch('src.tools.process_html.os.path.exists')
    @patch('src.tools.process_html.os.listdir')
    def test_process_html_without_splitting(self, mock_listdir, mock_exists, mock_file, mock_loader, mock_glob):
    ...
    def test_process_html_with_splitting(self, mock_chunk, mock_split, mock_listdir, mock_exists, mock_file, mock_loader, mock_glob):
    ...
    @patch('src.tools.process_html.os.path.exists')
    @patch('src.tools.process_html.os.listdir')
    def test_process_html_missing_source_in_dict(self, mock_listdir, mock_exists, mock_file, mock_loader, mock_glob):
    ...
    def test_process_html_split_without_chunk_size_raises_error(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('builtins.open', mock_open(read_data='{"test.html": "https://example.com"}')):
    ...
    @patch('src.tools.process_html.glob.glob')
    @patch('src.tools.process_html.UnstructuredHTMLLoader')
    @patch('builtins.open', new_callable=mock_open, read_data='{"file1.html": "https://example1.com", "file2.html": "https://example2.com"}')
    @patch('src.tools.process_html.os.path.exists')
    @patch('src.tools.process_html.os.listdir')
    def test_process_html_multiple_files(self, mock_listdir, mock_exists, mock_file, mock_loader, mock_glob):
    ...
    @patch('src.tools.process_html.os.path.exists')
    @patch('src.tools.process_html.os.listdir')
    def test_process_html_logs_warning_for_missing_source(self, mock_listdir, mock_exists, mock_file, mock_loader, mock_glob, mock_logging):
    ...
    def test_process_html_metadata_transformation(self):
        ...
        with patch('src.tools.process_html.glob.glob') as mock_glob:
            with patch('src.tools.process_html.UnstructuredHTMLLoader') as mock_loader:
                with patch('builtins.open', mock_open(read_data='{"test.html": "https://example.com"}')):
                    with patch('src.tools.process_html.os.path.exists', return_value=True):
                        with patch('src.tools.process_html.os.listdir', return_value=['test.html']):
                            mock_glob.return_value = ['./nested/path/test.html']
                            
                            mock_doc = Mock()
                            mock_doc.metadata = {"source": "original_source", "other_key": "other_value"}
                            mock_doc.page_content = "Test content"
                            mock_loader_instance = Mock()
                            mock_loader_instance.load.return_value = [mock_doc]
                            mock_loader.return_value = mock_loader_instance
                            ...
                            assert "nested/path/test.html" in result[0].metadata["source"]
    ...
    def test_process_html_real_file_structure(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            html_file.write_text("<html><head><title>Test</title></head><body><h1>Test Content</h1></body></html>")
            
            # Mock the source_list.json
            with patch('builtins.open', mock_open(read_data='{"docs/html/test.html": "https://example.com"}')):
                with patch('src.tools.process_html.UnstructuredHTMLLoader') as mock_loader:
                    mock_doc = Mock()
                    mock_doc.metadata = {"source": "test.html"}
                    mock_doc.page_content = "Test Content"
