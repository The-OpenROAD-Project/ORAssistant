...
from ..prompts.answer_prompts import summarise_prompt_template, tool_calling_prompt_template
...
class ToolNode:
    ...
    def get_node(self, state: AgentState) -> dict[str, list[str]]:
        ...
        if response != []:
            response = [item for sublist in response for item in sublist] if isinstance(response[0], list) else response
        if sources != []:
            sources = [item for sublist in sources for item in sublist] if isinstance(sources[0], list) else sources
        if urls != []:
            urls = [item for sublist in urls for item in sublist] if isinstance(urls[0], list) else urls
...
class RetrieverGraph:
    ...
    def agent(self, state: AgentState) -> dict[str, list[str]]:
        ...
        if self.inbuit_tool_calling:
            ...
            tool_choice_chain = (ChatPromptTemplate.from_template(tool_calling_prompt_template) | self.llm | JsonOutputParser())
            response = tool_choice_chain.invoke({'question': messages, 'tool_descriptions': self.retriever_tools, 'chat_history': state['chat_history']})
            
            if response is None:
                logging.warn("Tool selection response not found. Returning empty tool list.")
                return {'tools': []}
